...
class ThreeCandleBreakoutRetest(Strategy):
    ...
    min_body_atr_ratio   = 0.3   # each setup candle body must be > 30% of ATR
    breakout_atr_ratio   = 0.3   # breakout close must exceed level by > 30% ATR
...
def print_report(stats):
    SEP = "─" * 55
    print(f"\n{'═'*55}")
    print("  XAUUSD 15m  |  3-Candle Breakout + Retest  v2")
    print(f"{'═'*55}")
...
def plot_results(stats, save_path="equity_curve.png"):
    ...
    fig.suptitle("XAUUSD 15m  |  3-Candle Breakout + Retest  v2",
